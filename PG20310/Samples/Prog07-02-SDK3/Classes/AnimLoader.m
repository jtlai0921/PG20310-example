...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qualifiedName attributes:(NSDictionary *)attributeDict
{
	...
	if ([elementName isEqualToString:@"animation"]) {
		...
		if (tk) {
			...
			} else if ([type isEqualToString:@"yawpitchroll"]) {
				currAnimKeyType = AKT_YAWPITCHROLL;
			} else {
				currAET = AET_ERROR;
			}
		} else {
		...
		if (currAET == AET_ANIMKEY) {
			...
			} else if (currAnimKeyType == AKT_YAWPITCHROLL) {
				currAnimKey = [[AnimYawPitchRollKey alloc] initWithTime:tm];
			}
		}
	}
}
	...
	switch (currAET) {
		case AET_ANIMKEY:
			if (currAnimKeyType == AKT_POSITION3) {
				...
				if ([Loader parseYawPitchRoll:&ypr from:string]) {
					((AnimYawPitchRollKey *)currAnimKey).ypr = ypr;
				} else {
					NSLog(@"ERROR!!! string=\"%@\"", string);
				}
			}
