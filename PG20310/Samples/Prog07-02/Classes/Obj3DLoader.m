...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qualifiedName attributes:(NSDictionary *)attributeDict
{
	...
	if ([elementName isEqualToString:@"object"]) {
		...
		currOET = OET_MATRIX;
	} else if ([elementName isEqualToString:@"animfile"]) {
		NSString *src;
		...
		if (src) {
			...
			Animation *anm;
			animLoader = [[AnimLoader alloc] init];
			anm = [animLoader load:src];
			if (anm) {
				[currObj setAnimation:anm];
			}
			currOET = OET_ANIMFILE;
			[animLoader release];
		} else {
			currOET = OET_ERROR;
		}
